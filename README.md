...
- `<BTree-file>` is the B-Tree file generated by the `GeneBankCreateBTree` program
...
information. This information should be stored at the beginning of the B-Tree file.
...
`AATGC`, we will search for both `AATGC` and its complement `TTACG` to get the result.
...
Start off by running tests on your machine. If you do need to run them on `onyx` please only run the
smallest test (`test0.gbk`) to avoid overloading the `pnyx` server.

We will setup [Amazon AWS](https://aws.amazon.com/) accounts for each student so that we can
run larger tests in the cloud. **Running our tests on AWS is required so we can all get
